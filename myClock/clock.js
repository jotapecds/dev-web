...
const black = style.getPropertyValue("--cblack");

// Roman numbers and date color.
const black1 = style.getPropertyValue("--cblack1");

// Decimal numbers color.
const black2 = style.getPropertyValue("--cblack2");
...
function drawClock(place) {
  ...
  img.decode().then(() => {
    ...
    logo.src = "./img/escudo-remo-fla.png";
    logo.decode().then(() => {
      // Translate the center of the logo
      // to the center of the canvas.
  ...
  drawClock.romans.map((n, i) => {
    ...
    var coord = polar2Cartesian(0.85 * clockRadius, i * fiveMin * (-1));
  ...
  drawClock.decimals.map((n, i) => {
    ...
    var coord = polar2Cartesian(1.01 * clockRadius, i * fiveMin * 0.5 * (-1));
    // translate to the center of the canvas
    coord = translate(coord, center);
    context.fillText(n.txt, coord.x, coord.y);
  });
...
drawClock.decimals = Array.from(Array(24), (_, i) => {
  return { txt: String(i), c: black2 };
});
drawClock.decimals[0].txt = "24";
drawClock.decimals[6].c = red;
drawClock.decimals[18].c = red;
...
var runAnimation = (() => {
  ...
  function drawHandles() {
    ...
    clock_handles.map((handle) => {
      ...
      ctx.beginPath();
      //handle.time2Angle = handle.time2Angle * (-1) + 180;
